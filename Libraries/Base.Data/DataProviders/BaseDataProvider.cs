...
namespace Base.Data.DataProviders
{
    public abstract partial class BaseDataProvider : IMappingEntityAccessor
    {
        ...
        protected static ConcurrentDictionary<Type, CRMEntityDescriptor> EntityDescriptors { get; } = new ConcurrentDictionary<Type, CRMEntityDescriptor>();
        ...
        public virtual CRMEntityDescriptor GetEntityDescriptor(Type entityType)
        {
            return EntityDescriptors.GetOrAdd(entityType, t =>
            {
                ...
                return new CRMEntityDescriptor
                {
                    EntityName = tableName,
                    SchemaName = builder.Expression.SchemaName,
                    Fields = builder.Expression.Columns.Select(column => new CRMEntityFieldDescriptor
                    {
                        Name = column.Name,
            ...
            static Type getPropertyTypeByColumnName(Type targetType, string name)
            {
                var (mappedType, _) = Array.Find(targetType
                    ...
                    pi => name.Equals(NameCompatibilityManager.GetColumnName(targetType, pi.Name))).PropertyType.GetTypeToMap();
